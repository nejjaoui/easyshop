		...
		$.each( r, function(i,b) {
			if ( u(b) ) {
				...
				$.each( svx.settings.filters.settings[b.type], function(n,p) {
					s[i][n] = u(svx.settings.filters.val[i])&&u(svx.settings.filters.val[i][n])?svx.settings.filters.val[i][n]:(u(svx.settings.filters.settings[b.type].default)?svx.settings.filters.settings[b.type].default:'');
				} );
